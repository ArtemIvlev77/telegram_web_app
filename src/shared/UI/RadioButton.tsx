...
const RadioButton:FC<RadioButtonProps> = ({id, name, checked, changeHandler, clickHandler}) => {
	return (
		<div className="w-full" onClick={clickHandler}>
			...
			<label htmlFor={name} className="w-full block cursor-pointer select-none rounded-xl p-2 text-center peer-checked:bg-blue-500 peer-checked:font-bold peer-checked:text-white">{id}</label>
		</div>
	);
};
